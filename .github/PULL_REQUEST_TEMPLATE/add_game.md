...
- [ ] Added `steam_app_id` (from the Steam store URL) for box art, or `art_urls` with official publisher art if the game isn't on Steam
- [ ] Added `genres` (values from `GENRES` in `registry.rs`)
